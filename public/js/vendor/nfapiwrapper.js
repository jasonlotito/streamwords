...
function NowFinityApi(customHost, $iframe = null) {
    ...
    function requestAuth() {
        return new Promise((resolve, reject) => {
            ...
            let messageHandler = function (message) {
                if (message.origin === host.origin && typeof message.data === 'object' && message.data.channelId) {
                    localStorage.setItem('nf_channelId', message.data.channelId);
                    localStorage.setItem('nf_channelSignature', message.data.channelSignature);
    ...
    function get(endpoint, params, extraOptions) {
        return new Promise((resolve, reject) => {
            $.ajax({
                url: host.origin + '/api/' + endpoint + (params ? "?" : "") + (new URLSearchParams(params)).toString(),
                type: "GET",
